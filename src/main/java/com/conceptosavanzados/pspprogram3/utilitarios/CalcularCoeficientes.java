...
public class CalcularCoeficientes 
{
     ...
     * @param listaDeDatosXY Mapa con el listado de datos X y Y 
     * @param Xk parametro tipo double con el valor a estimar
     * @return String con el calculo de todos los coeficientes
     */
    public String Calcular(HashMap<String, LinkedList<Double>> listaDeDatosXY, Double Xk)
    {        
        setValoresX(listaDeDatosXY.get("X"));
        setValoresY(listaDeDatosXY.get("Y"));
        setXk(Xk);
        calcularB1();
        calcularB0();
        calcularYk();
